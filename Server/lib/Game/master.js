	...
	MainDB.ready = function(){
		...
		if(Const.IS_SECURED || Const.WAF.GAME) {
			...
			HTTPS_Server = https.createServer(options)
				.listen(global.test ? (Const.WAF.GAME ? Const.WAF.PORT : Const.TEST_PORT) : process.env['KKUTU_PORT']);
			Server = new WebSocket.Server({server: HTTPS_Server});
		} else {
			Server = new WebSocket.Server({
				port: global.test ? (Const.WAF.GAME ? Const.WAF.PORT : Const.TEST_PORT) : process.env['KKUTU_PORT'],
