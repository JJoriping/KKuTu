	...
	switch(cmd){
		...
		case "info_room":
			if(temp = ROOM[value]){
				...
				if(DIC[id]) DIC[id].send('tail', { a: i ? "irX" : "ir", rid: temp.id, id: id, msg: { pw: temp.password, players: temp.players } });
			}
			return null;
		case "info_user":
			if(temp = DIC[value]){
				...
				temp.send('test');
				if(DIC[id]) DIC[id].send('tail', { a: i ? "iuX" : "iu", rid: temp.id, id: id, msg: temp.getData() });
			}
			return null;
		case "mute":
			if(temp = DIC[value]){
				temp.socket.send(`{"value":"<script>$('.ChatBox input').last().hide(); $('.ChatBox button').last().hide(); $('.ChatBox #mute').show(); setTimeout(function(){ $('#Yell').hide(); $('#Yell').text(null); $('.ChatBox .chat-item').last().remove(); });</script>", "type":"yell"}`);
			}
			return null;
		case "unmute":
			if(temp = DIC[value]){
				temp.socket.send(`{"value":"<script>$('.ChatBox input').last().show(); $('.ChatBox button').last().show(); $('.ChatBox #mute').hide(); setTimeout(function(){ $('#Yell').hide(); $('#Yell').text(null); $('.ChatBox .chat-item').last().remove(); });</script>", "type":"yell"}`);
			}
			return null;
		case "dump":
