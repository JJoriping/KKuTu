		...
		if(nickname){
			...
			MainDB.users.findOne([ 'nickname', nickname ]).on(function(data){
				MainDB.users.findOne([ '_id', req.session.profile.id ]).on(function(requester){
					...
					if(GLOBAL.NICKNAME_POLICY.LIMIT.ENABLED && new Date() < new Date(changedDate.setDate(changedDate.getDate() + Number(GLOBAL.NICKNAME_POLICY.LIMIT.TERM)))) return res.send({ error: 457 });
